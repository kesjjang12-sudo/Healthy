...
## 선택지 카드(AskUserQuestion)를 쓰지 않는다

오너는 폰의 Claude Code 앱으로 본다. 거기서 선택지 카드가 대화 내용 위에
겹쳐 그려져 글이 안 읽힌다(2026-08-19 화면 녹화로 확인). 답을 고르고 나서도
카드가 계속 남아 방해한다.

물어볼 게 있으면 **그냥 본문에 한국어로 적는다.** 대부분은 애초에 물을
필요가 없다 — 되돌릴 수 있는 일이면 판단해서 진행하고 무엇을 골랐는지
결과에 적으면 된다. 정말 오너만 정할 수 있는 것(돈이 나가는 결정, 지우면
못 되살리는 것)만 본문으로 묻고 멈춘다.
