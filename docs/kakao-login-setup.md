...
### ⚠️ 2026-08 콘솔 개편 — 메뉴 경로가 바뀌었다

카카오가 콘솔을 개편해서, 인터넷과 예전 문서에 있는 경로가 안 맞는다.
공식 문서(`docs/ko/app-setting/app`, `docs/ko/kakaologin/prerequisite`)로
2026-08-18 에 확인한 현재 경로는 아래와 같다.

| 옛 경로 | 현재 경로 |
|---|---|
| 제품 설정 → 카카오 로그인 → **보안** | **없어졌다.** 앱 → 플랫폼 키 → REST API 키 안 |
| 제품 설정 → 카카오 로그인 → Redirect URI | 앱 → 플랫폼 키 → REST API 키 → 리다이렉트 URI |
| 앱 설정 → 앱 키 | 앱 → 플랫폼 키 |
| 앱 설정 → 플랫폼 → Web 등록 | 별도 단계로 안 나온다 (제품 링크 관리 → 웹 도메인) |
| 제품 설정 → 카카오 로그인 → 활성화 설정 | 카카오 로그인 → 사용 설정 → 상태 ON |

## 2. REST API 키와 Client Secret 받기

둘 다 **앱 → 플랫폼 키 → REST API 키** 한 화면에 있다.

- `REST API 키` 값 → Supabase 의 `client_id`
  (네이티브 앱 키·JavaScript 키·어드민 키가 아니라 **REST API 키**다)
- 같은 화면의 `클라이언트 시크릿` → **코드 생성 → 저장 → 활성화 ON → 저장**
  → Supabase 의 `client_secret`
  (생성만 하고 저장·활성화를 안 하면 Supabase 가 인증에 실패한다)

## 3. Redirect URI 등록 (가장 자주 틀리는 곳)

**앱 → 플랫폼 키 → REST API 키 → 리다이렉트 URI** 에 아래를 **그대로** 넣는다.
...
- **카카오 로그인 → 동의항목**:
  ...
  - `카카오계정(이메일)` — **켜야 한다.** 이유는 아래(정정됨).

### ⚠️ 정정 (2026-08-18) — 이메일은 꺼도 되는 게 아니라 켜야 한다

예전 판에는 "이메일을 안 받으면 사업자등록 없이 출시할 수 있다"고 적혀 있었다.
**틀렸다.** 실제로 꺼 봤더니 로그인이 `KOE205` 로 막혔다.

Supabase 의 카카오 제공자는 `account_email` 을 **무조건** 요청한다(앱 코드에서
끌 수 없다 — authorize 응답의 scope 로 확인). 우리 앱이 이메일을 안 쓰는 것과
무관하게, 카카오에서 이 항목이 꺼져 있으면 "설정하지 않은 동의 항목을 요청"
으로 거부된다. Supabase 쪽에도 올라와 있는 문제다(supabase/supabase#36878).

`account_email` 은 **비즈앱으로 전환한 앱만** 쓸 수 있으므로, 결국 비즈앱
전환이 필요하다. 전화번호(키오스크 매칭)도 비즈앱이 필요하니 한 번에 한다
— `docs/카카오-비즈앱-전화번호.md` 참고.

Supabase 의 **"Allow users without an email"** 은 그대로 켜 두는 게 안전하다.
카카오톡을 안 쓰는 계정 등 이메일이 비어 오는 경우가 있다.
