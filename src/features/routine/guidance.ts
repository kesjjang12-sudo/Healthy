...
export const HOW_TO_STEPS: readonly string[] = [
  '자리에 앉아 등과 허리를 등받이에 붙여요.',
  '손잡이나 발판을 잡고, 팔다리가 너무 굽지 않게 자리를 맞춰요.',
  '힘을 주면서 둘 셀 동안 밀고, 셋 셀 동안 천천히 돌아와요.',
  '밀 때 숨을 내쉬고, 돌아올 때 들이마셔요.',
  '한 세트가 끝나면 1분쯤 쉬었다가 다음 세트를 해요.',
];
...
export const CARDIO_HOW_TO_STEPS: readonly string[] = [
  '아주 천천히 시작해서 몸을 3분쯤 덥혀요.',
  '옆 사람과 말은 할 수 있지만 노래는 못 부를 정도의 속도를 유지해요.',
  '숨이 너무 차거나 어지러우면 속도를 낮추거나 잠시 멈춰요.',
  '끝나기 전 2~3분은 다시 천천히 속도를 줄여 마무리해요.',
];
...
export function weightHint(item: RoutineItem): string {
  if (item.target_weight === null) {
    return '기구에 무게가 없는 운동이에요. 몸으로만 천천히 하세요.';
  }
