...
-- 순서가 중요해서 하나로 합쳐 뒀다 — 나눠서 돌리면 뒤엣것이 앞엣것을
-- 필요로 해 실패한다.
--
-- 여러 번 돌려도 안전하다. 함수는 create or replace, 컬럼은
-- add column if not exists, 기구는 on conflict do update 라서
-- 이미 올라간 상태에서 다시 돌려도 같은 결과가 된다.
--
-- 들어 있는 것:
--   1. join_gym            태블릿을 안 거친 계정에도 루틴이 나오게 한다
--   2. 전화번호로 계정 찾기  SMS 인증 로그인이 새 가입이 되지 않게 한다
--   3. 운동 카탈로그·기구 회전  사람마다 다른 기구를 배정한다
--   4. 기구 데이터 16종      3번이 고를 수 있는 실제 운동 목록
-- ═══════════════════════════════════════════════════════════════


-- ───────────────────────────────────────────────────────────────
-- [1/4] 20260813000001_join_gym_without_kiosk.sql
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- [2/4] 20260813000002_claim_account_by_verified_phone.sql
-- ───────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────
-- [3/4] 20260813000003_exercise_catalog_and_rotation.sql
-- ───────────────────────────────────────────────────────────────
