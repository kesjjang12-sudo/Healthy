...
1. **기구 앞 QR 스캔** — 지금은 "영상으로 보기" 버튼이 바로 링크를 여는데, 기구 앞에
   붙은 실물 QR 을 찍어서 그 기구의 운동 화면으로 바로 들어가는 흐름은 아직 없다
2. **실제 SMS 인증** — 전화번호 로그인은 지금 QR 페어링(물리적 근접)으로 신원을 대신
   확인한다. Twilio 등으로 실제 SMS OTP 를 붙이면 폰 소유 증명이 완성된다
3. **카카오 전화번호 스코프** — 사업자등록 + 카카오 심사를 받으면, 카카오 로그인만으로
   전화번호까지 자동으로 받아 QR 페어링 없이도 매핑이 끝난다. `oauth.ts` 에 훅 지점만
   남겨 뒀다
4. **유산소/러닝 기록** — 분석 탭은 지금 근력운동 완료 기록만 본다. 러닝머신 등 유산소
   기구의 거리·시간을 기록하려면 새 테이블과 입력 UI가 필요하다
5. 트레이너 검수 후 `goal_blocks` / `pain_area_rules` / `age_modifiers` 수치 조정
